...
?>
    ...
    <head>
        ...
        <style>
            body {
                ...
                align-items: center;
                min-height: 100vh;
                background: linear-gradient(135deg, #0A2558 0%, #17684f 100%);
                margin: 0;
            }
    ...
    <body>
        <div class="box">
            ...
            <a href="students/dash.php" class="btn btn-primary">Go to Dashboard</a>
...
if (isset($_POST["register"])) {
    ...
    if (mysqli_num_rows($school_check) == 0) {
        $error_msg = "Invalid school selection. Please select a valid school.";
    } else {
        // Check if username already exists
        $check_user = mysqli_query($conn, "SELECT uname FROM student WHERE uname = '$uname'");
        if (mysqli_num_rows($check_user) > 0) {
            $error_msg = "Username already exists. Please choose a different username.";
        } else {
            // Check if email already exists
            $check_email = mysqli_query($conn, "SELECT email FROM student WHERE email = '$email'");
            if (mysqli_num_rows($check_email) > 0) {
                $error_msg = "Email already registered. Please use a different email.";
            } else {
                // Insert new student
                $sql = "INSERT INTO student (fname, email, dob, gender, uname, pword, school_id) 
                        VALUES ('$fname', '$email', '$dob', '$gender', '$uname', '$pword', $school_id)";
    ...
    <div class="container" id="container">
        <div class="form-container log-in-container">
            <form action="#" method="post">
                ...
                <p style="margin-top: 15px; font-size: 13px;">Already have an account? <a href="login_student.php" style="color: #0A2558; font-weight: bold;">Login here</a></p>
            </form>
        </div>
        <div class="overlay-container">
            <div class="overlay">
                <div class="overlay-panel overlay-right">
                    ...
                    <button style="background-color:#ffffff;border-color:black;"><a href="register_teacher.php">Continue</a></button>
                </div>
            </div>
        </div>
    </div>
