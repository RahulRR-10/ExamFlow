...
<body>
            ...
            <div class="left-column">
                ...
                <div class="photo-container">
                    ...
                    <div class="photo-meta">
                        <p>📅 Activity Date: <strong><?= date('F d, Y', strtotime($submission['activity_date'])) ?></strong></p>
                        <?php if ($submission['photo_taken_at']): ?>
                        <p>📷 Photo Taken: <strong><?= date('F d, Y H:i:s', strtotime($submission['photo_taken_at'])) ?></strong></p>
                        <?php endif; ?>
                        <p>📍 GPS: <strong><?= $submission['gps_latitude'] ?>, <?= $submission['gps_longitude'] ?></strong></p>
                    </div>
                </div>
            ...
            <div class="right-column">
                ...
                <div class="location-alert <?= $submission['location_match_status'] ?>">
                    ...
                    <?php elseif ($submission['location_match_status'] === 'mismatched'): ?>
                        <strong>⚠️ Location Mismatch</strong>
    ...
    <script>
        ...
        L.marker([photoLat, photoLng], {icon: photoIcon})
            .addTo(map)
            .bindPopup('<strong>📷 Photo Location</strong><br>' + photoLat.toFixed(6) + ', ' + photoLng.toFixed(6));

        // School location marker and radius
        if (schoolLat && schoolLng) {
            const schoolIcon = L.divIcon({
                html: '<div style="background:#667eea;width:24px;height:24px;border-radius:50%;border:3px solid white;box-shadow:0 2px 5px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;font-size:12px;">🏫</div>',
            ...
            L.marker([schoolLat, schoolLng], {icon: schoolIcon})
                .addTo(map)
                .bindPopup('<strong>🏫 School Location</strong>');
